...
(function () {
  ...
  function initialState() {
    return {
      ...
      simulationRuns: [],
      jobs: [],
      currentRole: localStorage.getItem('ps-current-role-southlake') || 'Admin',
      deletedProductIds: []
    };
  }
  ...
  function productDetailFrom(product, sourceDetail) {
    ...
    return Object.assign(source, {
      ...
      studios: source.studios || [
        { id:'coverage', name:'Coverage Guide', icon:'umbrella', status:'partial', summary:'Draft configuration', href:'coverage-studio.html' },
      ...
      checklist: source.checklist || [
        { studio:'Coverage Guide', status:'warn', note:'Review required' },
...
// 50 states + DC — used as the nationwide default jurisdiction set for
// SouthLake products, which skip the Define Jurisdiction step entirely.
...
const STUDIO_NAV_CHAIN = [
  { id: 'coverage', file: 'coverage-studio.html', title: 'Class of Business Guide' },
...
function coverValidationIssues(cover) {
 ...
 function applyCoverCompletionFlags(covers) {
  ...
  function previousIncompleteStudio(studioId, productId, version) {
    const pid = productId || context().productId;
    const chain = enabledStudioChain(pid);
    const idx = chain.findIndex(s => s.id === studioId);
    if (idx <= 0) return null;
    for (let i = 0; i < idx; i++) {
      if (isStudioSkipped(pid, chain[i].id)) continue;
  ...
  function canLeaveCurrentStudio(destPage) {
    ...
    if (isStudioSkipped(context().productId, fromId)) return true;
  ...
  function updateSkipStudioLink(container, show, onSkip) {
    if (!container) return;
    let btn = container.querySelector('[data-studio-skip]');
    if (!show) {
      btn?.remove();
      return;
    }
    if (!btn) {
      ...
      btn.type = 'button';
      btn.dataset.studioSkip = '1';
      btn.className = 'btn btn-ghost btn-sm';
      btn.textContent = 'Skip for now';
      const nextBtn = container.querySelector('[data-studio-next]');
      const back = container.querySelector('#ctx-back-link, #back-product-btn, #savebar-back-btn');
      if (nextBtn) container.insertBefore(btn, nextBtn);
      else if (back) container.insertBefore(btn, back);
      else container.appendChild(btn);
    }
    btn.onclick = onSkip;
  }

  function skipCurrentStudio(productId, version, studioId) {
    markStudioSkipped(productId, studioId);
    const next = nextStudioInChain(productId, studioId);
    if (next) window.location.href = studioNavHref(productId, version, next);
    else refreshStudioNav();
  }
  ...
  function refreshStudioNav() {
    ...
    const studioReady = typeof calculateStudioCompletion === 'function'
      ...
      : count > 0;
    const skipped = isStudioSkipped(ctx.productId, studioId);
    const canProceed = studioReady || skipped;
    const next = canProceed ? nextStudioInChain(ctx.productId, studioId) : null;
  ...
  function jurisdictionSetupFor(productId) {
    ...
    let states = Array.isArray(product.jurisdictions) ? product.jurisdictions : [];
    // SouthLake has no Define Jurisdiction step in its product flow — a
    // product with no explicit jurisdiction data is nationwide by default,
    // so every US state/territory shows as auto-selected everywhere this
    // feeds (Class of Business Guide's states panel, jurisdiction gating).
    if (!states.length && !existing.length) {
      states = US_JURISDICTIONS.slice();
    }
    const byState = {};
    existing.forEach(row => { if (row?.state) byState[row.state] = row; });
    const from = isoDateValue(product.effectiveFrom) || isoDateValue(product.createdOn) || isoDateValue(new Date());
  ...
  // SouthLake lets a user move on before a studio is fully configured —
  // "Skip for now" marks it so later gating (previousIncompleteStudio,
  // canLeaveCurrentStudio) stops treating it as a blocker, without
  // pretending it's actually complete for completion % purposes.
  function skippedStudiosFor(productId) {
    const product = productById(productId);
    const detail = state.productDetails[productId];
    const raw = (product && product.skippedStudios) || (detail && detail.skippedStudios) || [];
    return Array.isArray(raw) ? raw : [];
  }

  function isStudioSkipped(productId, studioId) {
    return skippedStudiosFor(productId).includes(studioId);
  }

  function markStudioSkipped(productId, studioId) {
    const product = productById(productId);
    const detail = ensureProductDetail(productId);
    const next = Array.from(new Set([...skippedStudiosFor(productId), studioId]));
    if (product) product.skippedStudios = next;
    if (detail) detail.skippedStudios = next;
    persistProduct(product || { id: productId }, detail);
    addAudit('MODIFIED', `Skipped ${studioId} guide for now`, { productId, studio: studioId });
    return next;
  }
  ...
  function refreshStudioSummaries(productId, version) {
    ...
    const studios = [
      mark('coverage', 'Coverage Guide', 'umbrella', 'coverage-studio.html', (bundle.covers || []).length, 'covers'),
  ...
  function createProductFromWizard() {
    ...
    const product = {
      ...
      carrier: PS.data?.currentUser?.carrier || 'SouthLake Carrier',
      mga: [],
      code,
  ...
  if (PS.nav) {
    PS.nav.switchRole = function (role) {
      ...
      localStorage.setItem('ps-current-role-southlake', role);
    ...
    window.executeAddCover = function () {
      try {
        ...
        if (typeof COVERS === 'undefined') {
          PS.closeModal();
          return showResult('Cover not added', 'Coverage Guide did not finish loading. Refresh and try again.', { type:'error' });
        }
