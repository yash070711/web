...
(function () {
  ...
  PS.uwRuleStore = {
    ...
    viewSourceHref(rule, productId, version) {
      ...
      return map[rule?.sourceStudio === 'Coverage Guide' ? 'Class of Business' : rule?.sourceStudio] || `underwriting-studio.html?product=${encodeURIComponent(pid)}&rule=${encodeURIComponent(rule?.id || '')}`;
    },
